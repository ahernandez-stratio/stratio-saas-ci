...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SSO_REDIRECT_URL', defaultValue: 'https://bootstrap.saas.stratio.com/acs/api/v1/auth/login', description: 'DC/OS API Login URL to be redirected after successful login in platform')
	...
	string(name: 'DOCKER_API_VERSION', defaultValue: '1.23', description: 'Docker API Version for client')
    ...
    stages {
       ...
       stage ('Build & SonarQube analysis') {
            steps {
                sh 'mvn clean deploy sonar:sonar --settings /opt/ci/settings.xml'
            }
       }
